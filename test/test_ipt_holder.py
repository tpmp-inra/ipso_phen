...
class TestIptHolder(unittest.TestCase):
    def test_tools_folder(self):
        ...
        print(__file__)
        tool_fld = os.path.join(os.path.dirname(__file__), "..", "ip_tools", "")
        self.assertTrue(
            os.path.isdir(tool_fld), f"Missing root ip tools folder '{tool_fld}'",
        )

    def test_script_files(self):
        """Check that fo every file in ip_tools, there's a corresponding file in the test's folder"""
        ...
        script_list = [
            f
            for f in glob.glob(
                os.path.join(os.path.join("..", test_folder, "ip_tools", ""), "ipt_*.py"),
                recursive=True,
            )
