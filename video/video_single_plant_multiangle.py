...
EXPERIMENT = "21as_mappi4_1911".lower()
    ...
    for main_angle_image_ in main_angle_image_list_:
        ...
        try:
            ...
            main_angle_wrapper_side.store_image(
                image=img_main_angle, text=SELECTED_ANGLES[0], force_store=True
            )
        ...
        for secondary_angle in SELECTED_ANGLES[1:]:
            secondary_angle_image_ = db_.query_one(
                command="SELECT",
                columns="FilePath",
                additional="ORDER BY date_time ASC",
                experiment=EXPERIMENT,
...
def main():
    ...
    ret = dbw.get_pg_dbw(dbw.DB_INFO_EXT_HD).query(
        command="SELECT DISTINCT", columns="Plant", Experiment=EXPERIMENT
    )
    plant_list_ = [item[0] for item in ret]

    num_cores = min([MAX_CORES, mp.cpu_count()])
    print(f"Starting building videos with {num_cores} cores")
    total_ = len(plant_list_)
    print_progress_bar(
        0, total=total_, prefix="Building videos:", suffix=f" {0}/{total_} Complete"
    )
    if num_cores > 1:
        pool = mp.Pool(num_cores)
        chunky_size_ = 1
        for i, _ in enumerate(
            pool.imap_unordered(build_video, (plant_ for plant_ in plant_list_), chunky_size_)
        ):
            print_progress_bar(
                i + 1,
                total=total_,
                prefix="Building videos (MP):",
                suffix=f" {i + 1}/{total_} Complete",
            )
    else:
        for i, plant in enumerate(plant_list_):
            build_video(plant)
            print_progress_bar(
                i + 1,
                total=total_,
                prefix="Building videos (SP):",
                suffix=f" {i + 1}/{total_} Complete",
            )
