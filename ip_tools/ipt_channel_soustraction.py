...
from ip_base.ip_common import TOOL_GROUP_PRE_PROCESSING_STR, ensure_odd
...
class IptChannelSubtraction(IptBase):
    ...
    def build_params(self):
        ...
        self.add_slider(
            name='alpha', desc='Weight of the first channel', default_value=100, minimum=0, maximum=100
        )
        self.add_channel_selector('bl', name='channel_2', desc='Channel 2')
        self.add_slider(
            name='beta', desc='Weight of the second channel', default_value=100, minimum=0, maximum=100
        )
        self.add_combobox(
        ...
        self.add_slider(name='min', desc='Threshold min value', default_value=0, minimum=0, maximum=255)
        self.add_slider(name='max', desc='Threshold max value', default_value=255, minimum=0, maximum=255)
        self.add_slider(
    ...
    def process_wrapper(self, **kwargs):
        ...
        try:
            ...
            if post_processing == 'normalize':
                ...
                self._wrapper.error_holder.add_error(f'Unknown postprocessing {post_processing}')
                res = False
                return

            wrapper.store_image(
                tmp, f'Subtraction_{self.input_params_as_str(exclude_defaults=True)}', text_overlay=True
            )

            if (min_ > 0) or (max_ < 255):
                mask, _ = wrapper.get_mask(tmp, '', min_, max_, median_filter_size=median_filter_size)
                ...
                self.result = tmp.copy(), mask.copy()
            else:
                self.result = tmp.copy(), None
