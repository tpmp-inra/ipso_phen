...
from ip_base.ip_common import TOOL_GROUP_ANCILLARY_STR
...
class IptEdgeDetector(IptBase):
    ...
    def process_wrapper(self, **kwargs):
        ...
        try:
            ...
            if self.get_value_of('enabled') == 1:
                ...
                if ((ch != sh) or (cw != sh)) and (self.get_value_of('source_file', 'source') == 'cropped_source'):
                    c = wrapper.crop_to_keep_roi(c)

                if operator_ == 'canny_scik':
                    edges = canny(c, sigma=canny_sigma, low_threshold=canny_first, high_threshold=canny_second)
                elif operator_ == 'canny_opcv':
                    edges = cv2.Canny(c, threshold1=canny_first, threshold2=canny_second)
                    ...
                    if kernel_size >= 3:
                        ...
                        wrapper.store_image(gauss, f'filtered_image_{self.input_params_as_str()}', text_overlay=True)
                ...
                if (
                    operator_ in ['laplacian', 'sobel', 'sobel_v', 'sobel_h', 'roberts', 'prewitt'] and
                    self.get_value_of('apply_threshold', default_value=1)
                ):
    ...
    @property
    def use_case(self):
        return [TOOL_GROUP_VISUALIZATION_STR, TOOL_GROUP_ANCILLARY_STR]
