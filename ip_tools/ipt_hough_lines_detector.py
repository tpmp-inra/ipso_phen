...
from ip_base.ip_common import (
    TOOL_GROUP_FEATURE_EXTRACTION_STR, TOOL_GROUP_IMAGE_INFO_STR, C_GREEN, C_LIME, C_RED, C_BLUE, C_LIGHT_STEEL_BLUE,
    all_colors_dict, build_color_steps
)
...
class IptHoughLines(IptBaseAnalyzer):
    ...
    def build_params(self):
        ...
        self.add_color_selector(
            name='discarded_color', desc='Discarded lines display color', default_value='fuchsia', enable_none=True
        )
        ...
        self.add_checkbox(name='add_lines_detail', desc='Add line details to output data', default_value=0)

    def add_line(self, src_img, index, x1: int, y1: int, x2: int, y2: int, kept_color, discarded_color) -> bool:
        ...
        cv2.putText(src_img, f"{index}|{line_angle:.2f}", (x1 - 5, y1 - 5), fnt[0], fnt[1], kept_color, 2)
        if min_angle <= line_angle <= max_angle:
            ...
            if self.get_value_of('add_lines_detail'):
                ...
                self.add_value(f'line_{index}_length', math.sqrt(abs(x2 - x1)**2 + abs(y2 - y1)**2), True)
            return True
        ...
        try:
            ...
            if self.get_value_of('source_file', 'source') == 'mask' and src_img is not None:
                src_img = wrapper.draw_image(
                    src_image=wrapper.current_image, src_mask=src_img, foreground='source', background='bw'
                )
            ...
            if is_probabilistic:
                ...
                if lines is not None:
                    for i, line in enumerate(lines):
                        x1, y1, x2, y2 = line[0]
                        if self.add_line(src_img, line_count, x1, y1, x2, y2, colors[i], discarded_color):
                            line_count += 1
            else:
                self.add_value('line_mode', 'full', True)
                lines = cv2.HoughLines(edges, rho=1, theta=np.pi / 180, threshold=votes_threshold)
                if lines is not None:
                    h, w = edges.shape[:2]
                    colors = build_color_steps(C_BLUE, C_LIGHT_STEEL_BLUE, len(lines))
                    for i, line in enumerate(lines):
            ...
            if build_mosaic:
                ...
                wrapper.store_image(canvas, f'lines_mosaic__{self.input_params_as_str()}', text_overlay=True)
