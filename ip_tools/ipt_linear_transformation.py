...
from ip_base.ip_common import C_WHITE, C_FUCHSIA, C_ORANGE
from ip_base.ipt_abstract_analyzer import IptBaseAnalyzer
from ip_base.ip_common import (
    TOOL_GROUP_FEATURE_EXTRACTION_STR, TOOL_GROUP_EXPOSURE_FIXING_STR, TOOL_GROUP_IMAGE_CHECK_STR
)
...
class IptLinearTransformation(IptBaseAnalyzer):
    ...
    def build_params(self):
        ...
        self.add_spin_box(name='beta', desc='Beta (brightness)', default_value=0, minimum=-255, maximum=255)
        self.add_spin_box(
            name='target_brightness', desc='Target brightness', default_value=0, minimum=0, maximum=255
        )
        self.add_spin_box(
        ...
        self.add_table_output(
            name='tb_output', desc=('Image', 'Brightness average', 'Contrast average', 'Brightness min/max')
        )
        ...
        try:
            ...
            if self.get_value_of('enabled') == 1:
                ...
                if method == 'alpha_beta':
                    ...
                    self.result = self.apply_gamma(img=img, gamma=self.get_value_of('alpha_gamma') / 100)
                    ...
                    if brg_calc != 'none':
                        ...
                        if roi is None:
                            ...
                            mask = roi.draw_to(
                                dst_img=np.full_like(a=img[:, :, 0], fill_value=0, dtype=np.uint8),
                                color=C_WHITE
                            )
                            self.wrapper.store_image(mask, 'avg_calc_mask')
                        avg_src, _ = wrapper.avg_brightness_contrast(img=img, mode=brg_calc, mask=mask)
                    else:
                        self.wrapper.error_holder.add_error('Please select brightness calculation method')
                        res = False
                        return
                    ...
                    if (((avg_src > target_brightness) and (apply_case in ['always', 'if_over'])) or
                        ((avg_src < target_brightness) and (apply_case in ['always', 'if_under']))):
                        if (method == 'gamma_target') or ((method == 'smart_target_brightness') and
                                                          (avg_src > target_brightness)):
                            self.result = self.apply_gamma(img=img, gamma=target_brightness / avg_src)
                        elif (method == 'alpha_beta_target') or ((method == 'smart_target_brightness') and
                                                                 (avg_src < target_brightness)):
                ...
                if brg_calc != 'none':
                    ...
                    br_dict = dict(
                        ...
                        **dict(
                            source_raw=(
                                f'{s_tuple[0][0][0]:.2f}', f'{s_tuple[1][0][0]:.2f}',
                                f'{round(s.min())}/{round(s.max())}'
                            ),
                            result=(
                                f'{r_tuple[0][0][0]:.2f}', f'{r_tuple[1][0][0]:.2f}',
                ...
                if (self.get_value_of('text_overlay') == 1) and (br_dict is not None):
                    wrapper.store_image(
                        self.result,
                        method,
                        text_overlay='\n'.join([f'{k.replace("_", " ")}: {v}' for k, v in br_dict.items()])
                    )
                else:
    ...
    def use_case(self):
        return [
            TOOL_GROUP_PRE_PROCESSING_STR, TOOL_GROUP_IMAGE_CHECK_STR, TOOL_GROUP_FEATURE_EXTRACTION_STR,
