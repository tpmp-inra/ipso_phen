...
class IptPcvBinaryThreshold(IptBase):
    ...
    def build_params(self):
        ...
        self.add_spin_box(
            name='threshold', desc='Threshold value (0-255)', default_value=0, minimum=0, maximum=255
        )
        self.add_spin_box(
            name='max_value',
        ...
        self.add_combobox(
            ...
            hint=
            '''"light" or "dark" (default: "light"). If object is lighter than the background then standard 
    ...
    def process_wrapper(self, **kwargs):
        ...
        try:
            if self.get_value_of('enabled') == 1:
                ...
                c = wrapper.get_channel(img, self.get_value_of('channel'), '', [], False, median_filter_size)

                self.result = pcv.threshold.binary(c, threshold, max_value, self.get_value_of('object_type'))
            ...
            res = False
            wrapper.error_holder.add_error(f"PCV Binary Threshold FAILED, exception: {repr(e)}")
        else:
            pass
        finally:
            return res
