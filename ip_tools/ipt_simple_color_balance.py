import cv2

from ip_base.ipt_abstract import IptBase
from ip_base.ip_common import (
    TOOL_GROUP_EXPOSURE_FIXING_STR, TOOL_GROUP_PRE_PROCESSING_STR, TOOL_GROUP_WHITE_BALANCE_STR
)


class IptSimpleWhiteBalance(IptBase):

    def build_params(self):
        self.add_color_space(default_value='RGB')
        self.add_slider(
            name='median_filter_size',
            desc='Median filter size (odd values only)',
            default_value=0,
            minimum=0,
            maximum=51
        )
        self.add_slider(name='min', desc='Threshold min %', default_value=2, minimum=0, maximum=100)
        self.add_slider(name='max', desc='Threshold max %', default_value=2, minimum=0, maximum=100)
        self.add_text_overlay(0)

    def process_wrapper(self, **kwargs):
        """
        Simple white balance: Performs a simple white balance

        Real time : No

        Keyword Arguments (in parentheses, argument name):
            * Color space (color_space): Convert source to color space
            * Median filter size (median_filter_size): Apply median filter if >0
            * Threshold min % (min): Lower cut percentage
            * Threshold max % (max): Upper cut percentage
            * Overlay text on top of images (text_overlay): Draw description text on top of images
        """
        wrapper = self.init_wrapper(**kwargs)
        if wrapper is None:
            return False

        text_overlay = self.get_value_of('text_overlay') == 1
        res = False
        try:
            min_ = self.get_value_of('min')
            max_ = self.get_value_of('max')
            color_space = self.get_value_of('color_space')

            res = self.extract_source_from_args()

            if color_space == 'HSV':
                res = cv2.cvtColor(res, cv2.COLOR_BGR2HSV)
            elif color_space == 'LAB':
                res = cv2.cvtColor(res, cv2.COLOR_BGR2LAB)

            res = wrapper.simplest_cb(res, (min_, max_))

            if color_space == 'HSV':
                res = cv2.cvtColor(res, cv2.COLOR_HSV2BGR)
            elif color_space == 'LAB':
                res = cv2.cvtColor(res, cv2.COLOR_LAB2BGR)

            self.result = res

            wrapper.store_image(res, f'simple_cb_{self.input_params_as_str()}', text_overlay=text_overlay)

        except Exception as e:
            res = False
            wrapper.error_holder.add_error(f'Simple color balance FAILED, exception: "{repr(e)}"')
        else:
            res = True
        finally:
            return res

    @property
    def name(self):
        return 'Simple white balance'

    @property
    def real_time(self):
        return True

    @property
    def result_name(self):
        return 'image'

    @property
    def output_kind(self):
        return 'image'

    @property
    def use_case(self):
        return [TOOL_GROUP_PRE_PROCESSING_STR, TOOL_GROUP_WHITE_BALANCE_STR, TOOL_GROUP_EXPOSURE_FIXING_STR]

    @property
    def description(self):
        return "Simple white balance: Performs a simple white balance.\nhttps://www.ipol.im/pub/art/2011/llmps-scb/article.pdf"
