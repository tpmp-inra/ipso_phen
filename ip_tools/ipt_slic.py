...
from ip_base.ip_common import DEFAULT_COLOR_MAP, TOOL_GROUP_CLUSTERING_STR
...
class IptSlic(IptBaseMerger):
    ...
    def build_params(self):
        ...
        self.add_slider(name='n_segments', desc='Segment count', default_value=3, minimum=1, maximum=250)
        self.add_slider(name='compactness', desc='Compactness', default_value=10, minimum=0, maximum=100)
    ...
    def process_wrapper(self, **kwargs):
        ...
        try:
            ...
            wrapper.store_image(
                slick_img, f'slic_vis_{self.input_params_as_str(exclude_defaults=True)}', text_overlay=True
            )

            self.print_segmentation_labels(slick_img, labels, dbg_suffix='slic', source_image=img.copy())
            ...
            res = False
            wrapper.error_holder.add_error(f'Slic failed, exception: "{repr(e)}"')
        else:
            res = True
        finally:
            return res
