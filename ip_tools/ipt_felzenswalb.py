...
class IptFelzenswalb(IptBaseMerger):
    ...
    def build_params(self):
        ...
        self.add_slider(name='min_size', desc="Min size", default_value=50, minimum=0, maximum=250)
        self.add_combobox(
            name='post_process',
            desc='Post process',
            default_value='none',
    ...
    def process_wrapper(self, **kwargs):
        ...
        try:
            ...
            wrapper.store_image(
                water_img, f'felzenszwalb_vis_{self.input_params_as_str()}', text_overlay=True
            )

            self.print_segmentation_labels(water_img, labels, dbg_suffix='felzenszwalb')
            ...
            res = False
            self._wrapper.error_holder.add_error(f'Felzenswalb FAILED, exception: "{repr(e)}"')
        else:
            res = True
        finally:
            return res
