...
class IptDemo(IptBase):
    ...
    def build_params(self):
        ...
        self.add_checkbox(name='checkbox', desc='A sample checkbox', default_value=0, hint='This is a hint')
        self.add_combobox(
            name='combobox',
            desc='Sample combobox',
            default_value='a',
            values=dict(a='First sample choice', b='Second sample choice', c='Third sample choice'),
    ...
    def process_wrapper(self, **kwargs):
        ...
        try:
            ...
            if p_in is not None and p_out is not None:
                p_out.update_output(
                    label_text=f'Text updated {self.update_count} times', output_value=p_in.value
                )
            ...
            if output_mode == 'raw':
                ...
                img = call_ipt(ipt_id='IptEdgeDetector', source=wrapper, **self.params_to_dict())
            else:  # This how we handle errors
                # If the error is added to the wrapper, it will be displayed in the main log
                wrapper.error_holder.add_error('Unknown output mode')
