...
class IptWatershedOpenCv(IptBaseMerger):
    ...
    def build_params(self):
        ...
        self.add_slider(
            name='dilate_iter', desc="Dilation iterations count", default_value=5, minimum=0, maximum=31
        )
        self.add_slider(name='min_area', desc='Min zone area ', default_value=1200, minimum=-1, maximum=50000)
        self.add_slider(
            name='distance_threshold', desc='Distance threshold', default_value=100, minimum=0, maximum=255
        )
        self.add_combobox(
            name='post_process',
            desc='Post process',
        ...
        try:
            ...
            if source_type == 'cropped_source':
                ...
                if keep_roi is not None:
                    ...
                    thresh = thresh[keep_roi.top:keep_roi.bottom, keep_roi.left:keep_roi.right]

            border = wrapper.dilate(thresh, 3, proc_times=dilate_iter)
            border = border - wrapper.erode(border, 3, proc_times=2)
            wrapper.store_image(border, f'border_{self.input_params_as_str()}', text_overlay=True)
            ...
            wrapper.store_image(
                water_img, 'watershed_vis_v2_{self._params_to_string(**kwargs)}', text_overlay=True
            )
            ...
            res = False
            wrapper.error_holder.add_error(f'Failed default process {repr(wrapper)}, exception: "{repr(e)}"')
        else:
            res = True
        finally:
            return res
