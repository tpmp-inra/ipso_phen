...
class IptImageSplitter(IptBase):
    ...
    def build_params(self):
        ...
        self.add_text_input(
            name='tray_id', desc='Tray Id', default_value='1', hint='Enter plaque Id, only alphanumerical values'
        )
        self.add_slider(name='line_count', desc='Line count', default_value=1, minimum=1, maximum=20)
        self.add_slider(name='column_count', desc='Column count', default_value=1, minimum=1, maximum=20)
        self.add_slider(
            name='padding_hor',
    ...
    def process_wrapper(self, **kwargs):
        ...
        try:
            ...
            folder = os.path.join(wrapper.folder_path, '')
            h, w = img.shape[:2]
            h_step, w_step = h // line_count, w // column_count

            for i in range(0, line_count):
                for j in range(0, column_count):
                    file_name = make_safe_name(f'gridsplit_{exp}_t{tray_id}-l{i+1}-c{j+1}-p{i*column_count + j + 1}')
                    slice_ = img[i*h_step + padding_ver:i*h_step + h_step - padding_ver, j*w_step +
                                 padding_hor:j*w_step + w_step - padding_hor]
                    wrapper.store_image(slice_, file_name)
                    if write_to_disc:
                        cv2.imwrite(os.path.join(folder, f'{file_name}{ext}'), slice_)
