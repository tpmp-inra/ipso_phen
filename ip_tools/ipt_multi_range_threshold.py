...
class IptMultiRangeThreshold(IptBase):
    ...
    def process_wrapper(self, **kwargs):
        ...
        try:
            ...
            res = False
            wrapper.error_holder.add_error(f"Multi range threshold FAILED, exception: {repr(e)}")
        else:
            res = True
        finally:
            return res
    ...
    def description(self):
        return "Performs range threshold keeping only pixels with values between min and max for up to 3 channels.\nMorphology operation can be performed afterwards"
