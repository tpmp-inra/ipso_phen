...
class IptPartialPosterizer(IptBase):
    ...
    def build_params(self):
        ...
        self.add_color_selector(
            ...
            enable_none=True,
            hint=
            'Replace value of pixels where blue is dominant and represents more than % value by selected color'
        )
        self.add_spin_box(
        ...
        self.add_color_selector(
            ...
            enable_none=True,
            hint=
            'Replace value of pixels where green is dominant and represents more than % value by selected color'
        )
        self.add_spin_box(
        ...
        self.add_color_selector(
            ...
            enable_none=True,
            hint=
            'Replace value of pixels where red is dominant and represents more than % value by selected color'
        )
        self.add_spin_box(
            name='post_red_value',
        ...
        try:
            ...
            if self.get_value_of('enabled') == 1:
                ...
                if blue_color != 'none':
                    ...
                    try:
                        where_more = cv2.bitwise_and((b > r).astype(np.uint8), (b > r).astype(np.uint8))
                ...
                if green_color != 'none':
                    np.seterr(divide='ignore')
                    try:
                        where_more = cv2.bitwise_and((g > r).astype(np.uint8), (g > b).astype(np.uint8))
                        where_percent = np.divide(
                            g.astype(np.float),
                            b.astype(np.float) + g.astype(np.float) + r.astype(np.float)
                        )
                ...
                if red_color != 'none':
                    np.seterr(divide='ignore')
                    try:
                        where_more = cv2.bitwise_and((r > b).astype(np.uint8), (r > g).astype(np.uint8))
                        where_percent = np.divide(
                            b.astype(np.float),
                ...
                if build_mosaic == 'channels':
                    ...
                    canvas = wrapper.build_mosaic(image_names=np.array(['source', 'partial_poster']))
