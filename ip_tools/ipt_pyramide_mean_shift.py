...
class IptPyramidMeanShift(IptBase):
    ...
    def build_params(self):
        self.add_slider(name='sp', desc='Spatial window radius', default_value=4, minimum=2, maximum=100)
        self.add_slider(name='sr', desc='Color window radius', default_value=10, minimum=0, maximum=100)
    ...
    def process_wrapper(self, **kwargs):
        ...
        try:
            ...
            wrapper.store_image(self.result, f'shifted_{self.input_params_as_str()}', text_overlay=True)
        except Exception as e:
            res = False
            self._wrapper.error_holder.add_error(f'Pyramide mean shift FAILED, exception: "{repr(e)}"')
        else:
            res = True
        finally:
            return res
