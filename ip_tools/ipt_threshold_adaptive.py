...
from ip_base.ip_common import TOOL_GROUP_THRESHOLD_STR, ensure_odd
...
class IptThresholdAdaptive(IptBase):
    ...
    def process_wrapper(self, **kwargs):
        ...
        Keyword Arguments (in parentheses, argument name):
            ...
            * Max value (max_value): Non-zero value assigned to the pixels for which the condition is satisfied
            * Adaptive method (method): Adaptive thresholding algorithm to use, see cv::AdaptiveThresholdTypes
            * Block size (block_size): Size of a pixel neighborhood that is used to calculate a threshold value for the pixel: 3, 5, 7, and so on.
        ...
        try:
            ...
            if self.get_value_of('enabled') == 1:
                ...
                if build_mosaic:
                    ...
                    canvas = wrapper.build_mosaic(
                        image_names=np.
                        array(['source_channel', f'adaptive_threshold_{self.input_params_as_str()}'])
                    )
            ...
            res = False
            self._wrapper.error_holder.add_error(f'adaptive threshold FAILED, exception: "{repr(e)}"')
        else:
            pass
        finally:
            return res
