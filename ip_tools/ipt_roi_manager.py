...
class IptRoiManager(IptBase):
    ...
    def reset(self, is_update_widgets: bool = True):
        super().reset(is_update_widgets)
        self.rois = []

    def copy(self, copy_wrapper: bool = True):
        if copy_wrapper:
            c = self.__class__(wrapper=self.wrapper, **self.params_to_dict())
        else:
            c = self.__class__(**self.params_to_dict())
        c.rois = self.rois[:]
        return c
    ...
    def execute(self, param, **kwargs):
        ...
        try:
            if param.name == 'draw_roi':
                ...
                img = resize_image(
                    img, width=ori_width // factor, height=ori_height // factor, keep_aspect_ratio=True
                )
    ...
    def process_wrapper(self, **kwargs):
        ...
        try:
            ...
            if self.result is not None:
                img = self.result.draw_to(dst_img=wrapper.current_image, line_width=wrapper.width // 200)
            else:
                img = wrapper.current_image
    ...
    def generate_roi(self, **kwargs):
        ...
        if roi_shape == 'rectangle':
            ...
            return shapes.CircleOfInterest(
                center=shapes.Point(left + radius_, top + radius_),
                radius=radius_,
                name=roi_name,
                tag=roi_type,
                target=tool_target
