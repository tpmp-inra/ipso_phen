...
class AbstractImageProcessor(ImageWrapper):
    ...
    def plot_image(self, img_dict: dict, destroy_window: bool = False) -> bool:
        ...
        try:
            ...
            self.error_holder.add_error(f'Unable to plot {img_dict["name"]}: "{repr(e)}"')
            res = False
        else:
            res = True
        finally:
    ...
    def default_process(self, **kwargs):
        ...
        try:
            ...
            res = False
            self.error_holder.add_error(f'Failed default process {mode}, exception: "{repr(e)}"')
        finally:
            self.print_images()
            return res
    ...
    def available_channels_as_tuple(self):
        if self.is_msp:
            res = [ci for ci in ipc.create_channel_generator(include_msp=True, include_chla=True)]
        elif self.is_vis:
