...
from ip_base.ip_common import (
    C_BLACK, C_LIGHT_STEEL_BLUE, C_SILVER, C_WHITE, all_colors_dict, enclose_image, resize_image, get_hr_channel_name,
    create_channel_generator, TOOL_GROUP_ANCILLARY_STR, ensure_odd, TOOL_GROUP_CLUSTERING_STR, TOOL_GROUP_DEMO_STR,
    TOOL_GROUP_DEFAULT_PROCESS_STR, TOOL_GROUP_FEATURE_EXTRACTION_STR, TOOL_GROUP_EXPOSURE_FIXING_STR,
    TOOL_GROUP_IMAGE_CHECK_STR, TOOL_GROUP_IMAGE_GENERATOR_STR, TOOL_GROUP_IMAGE_INFO_STR, TOOL_GROUP_MASK_CLEANUP_STR,
    TOOL_GROUP_PRE_PROCESSING_STR, TOOL_GROUP_ROI_DYNAMIC_STR, TOOL_GROUP_ROI_STATIC_STR, TOOL_GROUP_THRESHOLD_STR,
    TOOL_GROUP_VISUALIZATION_STR, TOOL_GROUP_WHITE_BALANCE_STR, TOOL_GROUP_UNKNOWN_STR
)
...
class IptParam(object):
    ...
    def update_label(self):
        ...
        if lbl is None:
            return False
        if isinstance(self.allowed_values, dict) or isinstance(self.allowed_values,
                                                               str) or (self.widget_type == 'spin_box'):
    ...
    def update_input(self, new_values=None):
        ...
        if isinstance(self.allowed_values, dict):
            if (new_values is
                not None) and isinstance(new_values, dict) and (self.allowed_values.keys() - new_values.keys() != {}):
            ...
            if self.allowed_values == (0, 1):
                ...
                if (new_values is not None) and isinstance(new_values, tuple) and (self.allowed_values != new_values):
                    self.allowed_values = new_values
                    widget.setMinimum(self.allowed_values[0])
                    widget.setMaximum(self.allowed_values[1])
                widget.setValue(int(self.value))
...
class IptParamHolder(object):
    ...
    def add_combobox(
        ...
        try:
            param = IptParam(name=name, desc=desc, default_value=default_value, allowed_values=values, hint=hint)
    ...
    def add_slider(
        self, name: str, desc: str, default_value: int = 0, minimum: int = 0, maximum: int = 100, hint: str = ''
    ) -> IptParam:
        param = IptParam(
            name=name, desc=desc, default_value=default_value, allowed_values=(minimum, maximum), hint=hint
        )
    ...
    def add_checkbox(self, name, desc, default_value, hint='') -> IptParam:
        ...
        """
        param = IptParam(name=name, desc=desc, default_value=default_value, allowed_values=(0, 1), hint=hint)
        param.widget_type = 'checkbox'
        return self.add(param)

    def add_text_input(self, name: str, desc: str, default_value: str = '-', hint: str = '') -> IptParam:
        param = IptParam(
            name=name, desc=desc, default_value=default_value, allowed_values='single_line_text_input', hint=hint
        )
    ...
    def add_text_output(
        ...
        param = IptParam(name=name, desc=desc, default_value=default_value, allowed_values=mode_, hint=hint)
        param.widget_type = mode_
        return self.add(param)

    def add_table_output(self, name: str, desc: tuple, default_value: dict = {}, hint: str = '') -> IptParam:
        param = IptParam(name=name, desc=desc, default_value=default_value, allowed_values='table_output', hint=hint)
    ...
    def add_label(self, name: str, desc: str, hint: str = '') -> IptParam:
        param = IptParam(name=name, desc=desc, default_value=desc, allowed_values='label', hint=hint)
    ...
    def add_color_selector(
        self, name='color', desc='Select color', default_value='light_steel_blue', hint='', enable_none: bool = False
    ) -> IptParam:
        if enable_none:
            values = {'none': 'none'}
        else:
    ...
    def add_channel_selector(
        self, default_value, name='channel', desc='Channel', hint: str = '', enable_none: bool = False
    ) -> IptParam:
        if enable_none:
            values = {'none': 'none'}
        else:
        ...
        values = {
            **values,
            **{channel_info[1]: get_hr_channel_name(channel_info[1]) for channel_info in create_channel_generator()}
        }
        param = IptParam(name=name, desc=desc, default_value=default_value, allowed_values=values, hint=hint)
        if enable_none:
    ...
    def add_arithmetic_operator(
        self, default_value='plus', name='operator', desc='Arithmetic operator', hint='Operator to use with operands'
    ) -> IptParam:
        param = IptParam(
            name=name,
            desc=desc,
    ...
    def add_source_selector(
        self, name: str = 'source_file', desc: str = 'Select source file type', default_value: str = 'source'
    ) -> IptParam:
        param = IptParam(
            name=name,
            desc=desc,
    ...
    def add_tool_target(self) -> IptParam:
        param = IptParam(name='tool_target', desc='Target IPT', default_value='none', allowed_values=dict(none='None'))
    ...
    def add_roi_shape(self, default_value='rectangle') -> IptParam:
        param = IptParam(
            ...
            default_value=default_value,
            allowed_values=dict(rectangle='Rectangle', circle='Circle, will be treated as rectangle for morphology')
        )
        param.kind = 'roi_shape_selector'
        return self.add(param)

    def add_roi_settings(
        self, default_name: str = 'unnamed_roi', default_type: str = 'other', default_shape: str = 'rectangle'
    ) -> IptParam:
    ...
    def add_edge_detector(self, default_operator: str = 'canny_opcv'):
        ...
        self.add_slider(name='canny_sigma', desc="Canny's sigma", default_value=2, minimum=0, maximum=20, hint='Sigma.')
        self.add_slider(
            name='canny_first',
        ...
        self.add_slider(name='kernel_size', desc='Kernel size', default_value=5, minimum=0, maximum=27)
        self.add_spin_box(
            name='threshold',
            desc='Threshold',
            default_value=130,
    ...
    def add_binary_threshold(self, add_morphology: bool = True):
        self.add_spin_box(name='min_t', desc='Threshold min value', default_value=0, minimum=0, maximum=255)
        self.add_spin_box(name='max_t', desc='Threshold max value', default_value=255, minimum=0, maximum=255)
        self.add_slider(
    ...
    def add_morphology_operator(self, default_operator: str = 'none'):
        ...
        self.add_slider(name='kernel_size', desc='Kernel size', default_value=3, minimum=3, maximum=101)
        self.add_combobox(
            name='kernel_shape',
            desc='Kernel shape',
        ...
        self.add_slider(name='proc_times', desc='Iterations', default_value=1, minimum=1, maximum=100)

    def add_button(self, name: str, desc: str, index: int = 0, hint: str = '') -> IptParam:
        param = IptParam(name=name, desc=desc, default_value=index, allowed_values='input_button', hint=hint)
        param.kind = 'button'
        self.add(param)
    ...
    def input_params(self, exclude_defaults: bool = False, excluded_params: tuple = (), forced_params: tuple = ()):
        return [
            p for p in self.gizmos
            if (p.is_input and not (exclude_defaults and p.is_default) and
                (p.name not in excluded_params)) or (p.name in forced_params)
        ]
...
class IptBase(IptParamHolder, ABC):
    ...
    def __str__(self):
        return f'{type(self).__name__}_' + self.input_params_as_str(
            exclude_defaults=True, excluded_params=("progress_callback",), forced_params=('channel',)
        )
                ...
                if len(img_lst_) > 0:
                    if kwargs.get('send_all_images', False):
                        for dic in ip.wrapper.image_list:
                            go_on = lcl_callback(
                                i + 1, tot_,
                                f'{ip.name}: {ip.input_params_as_str(exclude_defaults=True, excluded_params=("progress_callback",))}',
                                dic
                            )
                            if go_on is False:
                                return
    ...
    def do_channel_failure(self, channel):
        self.wrapper.store_image(self.wrapper.current_image, f'Missing {channel} channel', text_overlay=True)
        self.wrapper.error_holder.add_error(f'Missing {channel} channel', new_error_kind='source_issue')
    ...
    def to_uint8(self, img, normalize: bool = False):
        if str(img.dtype) == 'bool':
            ...
            img[img != 0] = 255
            return img
        elif (str(img.dtype) == 'float64') or (str(img.dtype) == 'float16') or (str(img.dtype) == 'int32'):
    ...
    def to_fuzzy(self, img):
        ...
        if str(img.dtype) == 'bool':
            img = img.astype(np.uint8)
            return img
        elif (str(img.dtype) == 'float64') or (str(img.dtype) == 'int32') or (str(img.dtype) == 'uint8'):
    ...
    def match_image_size_to_source(self, img, source_mode: str = 'source_file', ignore_list: tuple = ()):
    ...
    def extract_source_from_args(self, source_mode: str = 'source_file', ignore_list: tuple = ()):
        ...
        if source_type == 'source':
            ...
            self.wrapper.init_rois()
            src_img = self.wrapper.crop_to_roi(img=self.wrapper.current_image, roi=type(self).__name__)
        elif source_type == 'cropped_source':
            ...
            if ret:
                ...
                if masked_whole is not None:
                    ...
                    masked_whole = self.wrapper.apply_mask(self.wrapper.current_image, self.wrapper.mask, 'white')
                    src_img = masked_whole
            else:
    ...
    def apply_binary_threshold(self, wrapper, img, channel):
        ...
        mask, _ = wrapper.get_mask(
            src_img=img, channel=channel, min_t=min_, max_t=max_, median_filter_size=median_filter_size
        )
    ...
    def print_segmentation_labels(self, watershed_image, labels, dbg_suffix='', min_size=-1, source_image=None):
        ...
        for label in np.unique(labels):
            ...
            if LooseVersion(cv2.__version__) > LooseVersion('4.0.0'):
                contours_, _ = cv2.findContours(mask.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)[-2]
            else:
                _, contours_, _ = cv2.findContours(mask.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)[-2]
            ...
            cv2.putText(
                watershed_image, f"#{label}: {area_}", (x - 10, y), cv2.FONT_HERSHEY_SIMPLEX, 0.6, draw_color, 2
            )
            cv2.drawContours(source_image, [c], 0, draw_color, 4)
            cv2.putText(source_image, f"#{label}: {area_}", (x - 10, y), cv2.FONT_HERSHEY_SIMPLEX, 0.6, draw_color, 2)

        self._wrapper.store_image(watershed_image, f'{dbg_suffix}_vis_labels', text_overlay=True)
        self._wrapper.store_image(source_image, f'{dbg_suffix}_labels_on_source_image', text_overlay=True)
    ...
    def input_params_as_str(
        ...
        return ", ".join([
            f"{p.name}={p.str_value}" for p in self.input_params(
                exclude_defaults=exclude_defaults, excluded_params=excluded_params, forced_params=forced_params
            )
        ])
    ...
    def input_params_as_html(
        ...
        return f'<ul>' + ''.join(
            f'<li>{p.name}={p.str_value}</li>' for p in self.input_params(
                exclude_defaults=exclude_defaults, excluded_params=excluded_params, forced_params=forced_params
            )
        ) + '</ul>'
    ...
    def code_apply_roi(self, print_result=None, white_spaces=''):
        ...
        params_ = f",\n{ws}".join([f"{p.name}={p.str_value}" for p in self.input_params(exclude_defaults=True)])
    ...
    def code_body(self, **kwargs):
        ...
        wrapper_ = '{file}'
        if use_with_clause:
            ws = ''.join([' ' for _ in range(0, len(f'{white_spaces}with {type(self).__name__}('))])
        else:
            ws = ''.join([' ' for _ in range(0, len(f'{white_spaces}ipt_res = ipt.process_wrapper('))])
        params_ = f",\n{ws}".join([f"{p.name}={p.str_value}" for p in self.input_params(exclude_defaults=True)])
        ...
        if use_with_clause:
            ...
            code_ += f'{white_spaces}{add_ws}{add_ws}' + 'print(f"Process error: {str(wrapper.error_holder)}")\n'
        else:
            ...
            code_ += f'{white_spaces}    ' + 'print(f"Process error: {str(ipt.wrapper.error_holder)}")\n'
