...
_023_EXPERIMENT = "023s1807_bti".lower()
...
class Ip23s1807Bti(BaseImageProcessor):
    @staticmethod
    ...
    def check_source(self):
        res = super(Ip23s1807Bti, self).check_source()
        return res

    def init_csv_data(self, source_image):
        if self.is_empty_pot:
            _, _, id_, tag = self.plant.split("_")
            self.csv_data_holder.update_csv_value("plant_id", id_)
            self.csv_data_holder.update_csv_value("plant_genotype", tag.lower())
            self.csv_data_holder.update_csv_value("treatment", "none")
        else:
            _, _, id_, genotype_, treatment = self.plant.split("_")
            self.csv_data_holder.update_csv_value("plant_id", id_)
            self.csv_data_holder.update_csv_value("plant_genotype", genotype_)
            self.csv_data_holder.update_csv_value("treatment", treatment)
