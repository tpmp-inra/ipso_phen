...
class TestIptHolder(unittest.TestCase):
    def test_tools_folder(self):
        ...
        print(__file__)
        tool_fld = os.path.join(os.path.dirname(__file__), "..", "ipapi.ipt", "")
        self.assertTrue(
            os.path.isdir(tool_fld), f"Missing root ip tools folder '{tool_fld}'",
        )

    def test_script_files(self):
        """Check that fo every file in ipapi.ipt, there's a corresponding file in the test's folder"""
        ...
        script_list = [
            f
            for f in glob.glob(
                os.path.join(os.path.join("..", test_folder, "ipapi.ipt", ""), "ipt_*.py"),
                recursive=True,
            )
