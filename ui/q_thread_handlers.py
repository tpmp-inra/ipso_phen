...
class IpsoRunnable(QRunnable):
    ...
    def __init__(self, **kwargs):
        ...
        self.signals_holder.on_feedback_log_object.connect(kwargs.get('on_feedback_log_object', None))
    ...
    def _script_progress_callback(self, step: int, total: int, msg: str, sender: object):
        self.signals_holder.on_script_progress.emit(step, total, msg, None if self.batch_process else sender)
        ...
        if os.path.isfile(file_path):
            ...
            if wrapper is None:
                self.error_list.add_error(
                    new_error_text='No image selected: Missing wrapper', new_error_kind='wrapper_error'
                )
    ...
    def _execute_param(self, image_dict):
        ...
        if self.ipt is None:
            ...
            self.signals_holder.on_feedback_log_object.emit('No ipt selected: Missing IPT', self.error_list)
            return False
        try:
            ...
            self.signals_holder.on_feedback_log_object.emit('Unable to execute tool button', self.error_list)
        else:
            res = True
        finally:
            return res
    ...
    def _process_image(self, image_dict):
        ...
        try:
            ...
            if hasattr(self.ipt, 'data_dict'):
                self.signals_holder.on_update_data.emit(self.ipt.data_dict)
            elif len(self.ipt.wrapper.csv_data_holder.data_list) > 0:
                self.signals_holder.on_update_data.emit(self.ipt.wrapper.csv_data_holder.data_list)
            ...
            self.error_list.add_error(new_error_text='Unable to process image', new_error_kind='ipt_error')
            self.error_list.add_error(new_error_text=repr(e), new_error_kind='ipt_error')
            if self.ipt is not None:
                self.error_list.add_error(f'Processing {self.ipt.name}', new_error_kind='ipt_error')
            if param_list:
            ...
            self.signals_holder.on_feedback_log_object.emit('Unable to process image', self.error_list)
        finally:
            if status_message:
    ...
    def _process_script(self, image_dict):
        ...
        try:
            ...
            if self.script is None:
                self.error_list.add_error(
                    new_error_text='No script selected: Missing script', new_error_kind='script_error'
                )
            ...
            self.signals_holder.on_feedback_log_object.emit('Unable to process script', self.error_list)
        finally:
            if status_message:
                self.signals_holder.on_ending.emit(res, status_message, log_message, log_object)
            return res
...
class IpsoGroupProcessor(QRunnable):
    ...
    def _run_process(self, file_name, luid: str = ''):
        start_time = timer()
        ipo = ipo_factory(
            file_name, self.options, force_abstract=self.script is not None, data_base=self.database
        )
        if ipo is not None:
            ...
            try:
                ...
                err_holder = ErrorHolder(
                    self, (
                        ...
                        dict(text=f'Failed to process image because {repr(e)}', type='process_error'),
                    )
                )
                self.signals_holder.on_image_ready.emit(ipo.build_mosaic(image_names=np.array(['error'])))
                self.signals_holder.on_log_str.emit(f'{ui_consts.LOG_EXCEPTION_STR}: {err_holder.to_html()}')
                err_holder = None
            else:
                if res:
                    try:
...
class IpsoMassRunner(QRunnable):
    ...
    def run(self):
        ...
        try:
                ...
                if groups_to_process_count > 0:
                    ...
                    for i, group in enumerate(groups_to_process):
                        item_worker = IpsoGroupProcessor(
                            ...
                            script=None if self.pipeline.script is None else self.pipeline.script.copy()
                        )
                        if not self.is_continue():
                            launch_state = 'abort'
                            return
            ...
            self.error_list.add_error(new_error_text='Mass processing', new_error_kind='mass_process_error')
            ...
            if launch_state != 'ok':
                self.signals_holder.on_feedback_log_str.emit(
                    ...
                    'Failed to launch mass process, cancelling queued jobs', False
                )
    ...
    def is_continue(self):
        ...
        if not self._is_continue and not self._reported_stop:
            self.signals_holder.on_feedback_log_str.emit('Stopping mass process, please wait...', '', True)
    ...
    def do_feedback(self, status_msg: str, log_msg: str, obj: object, use_status_as_log: bool) -> bool:
        if obj is None:
...
class IpsoCsvBuilder(QRunnable):
    ...
    def run(self):
        ...
        try:
            ...
            if steps_ > 0:
                self.signals_holder.on_feedback_log_str.emit(
                    'Building additional CSV files', ' --- Building additional CSV files ---', False
                )
                step_ = 0
                step_ += 1
                ...
                if self.group_by_series_id:
                    ...
                    if self.build_median_df:
                        df.groupby('series_id').median().merge(df).drop([
                            'series_id'
                        ], axis=1).drop_duplicates().sort_values(by=['plant', 'date_time']).to_csv(
                            os.path.join(self.pipeline.options.dst_path, f'{csv_sid_root_name}_median.csv')
                        )
                        step_ += 1
                        self.signals_holder.on_progress.emit(step_, steps_)
                    if self.build_mean_df:
                        df.groupby('series_id').mean().drop(
                            ...
                            os.path.join(self.pipeline.options.dst_path, f'{csv_sid_root_name}_mean.csv')
                        )
                        step_ += 1
                        self.signals_holder.on_progress.emit(step_, steps_)
                if self.group_by_hour:
                ...
                if self.group_by_day:
                    ...
                    if self.build_median_df:
                        df.groupby('date').median().drop_duplicates().sort_values(
                            ...
                            os.path.join(self.pipeline.options.dst_path, f'{csv_day_root_name}_median.csv')
                        )
                        step_ += 1
                        self.signals_holder.on_progress.emit(step_, steps_)
                    if self.build_mean_df:
                        df.groupby('date').mean().drop_duplicates().sort_values(by=['plant', 'date']).to_csv(
                            os.path.join(self.pipeline.options.dst_path, f'{csv_day_root_name}_mean.csv')
                        )
                        step_ += 1
                        self.signals_holder.on_progress.emit(step_, steps_)
                self.signals_holder.on_feedback_log_str.emit(
