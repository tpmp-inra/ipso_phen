...
Details [here](ipt_Clean_horizontal_noise_(WIP).md)
...
From scikit-image: Apply hysteresis thresholding to image.<br>            This algorithm finds regions where image is greater than high OR image is <br>            greater than low and that region is connected to a region greater than high.<br>            In other words, a pixel is accepted if its value is greater than the upper threshold, <br>            or its value is higher than the lower threshold and one of has already been accepted.  <br>
