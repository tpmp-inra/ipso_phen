...
class ImageList:
    ...
    def _filter_extensions(self):
        ...
        """
        file_list = [
            os.path.join(root, name) for fld in self._folders_paths
            for root, _, files in os.walk(fld) for name in files if name.lower().endswith(self.extensions)
        ]
    ...
    def _apply_actions(self, file_list, actions, groups, flatten):
        ...
        if actions[0] == 'keep_lasts':
            ...
            while True:
                ...
                for item in file_list:
                    if item['key'][main_key] == current_sub_key and item['wrappers'][0].date > best_date:
                        best_date = item['wrappers'][0].date
                        best_group = item
    ...
    def filter_db(self, input_list, masks, flat_list_out=True):
        ...
        filters_, post_filters_, groups_, actions_, pick_, exclusions_, forced_, cut_at_date_ = self._demux_commands(
            masks
        )
    ...
    def filter(self, masks, flat_list_out=True):
        ...
        if not self._folders_paths:
            ...
            filters_, post_filters_, groups_, actions_, pick_, exclusions_, _, _ = self._demux_commands(masks)
